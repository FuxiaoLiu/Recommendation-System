...
class _TRAINER(object):
    ...
    def f_get_pred(self, network, user_ids, item_ids):
        ...
        user_logits = network.m_tag_user_embedding(user_x)
        item_logits = network.m_tag_item_embedding(item_x)
    ...
    def f_train_epoch(self, train_data, network, optimizer, logger_obj):
        ...
        network.train()
        for user_attr, item_attr, rating_batch, user_batch, item_batch in train_data:
            pos_tag_batch = user_attr
            neg_tag_batch = item_attr

            pos_tag_batch_gpu = pos_tag_batch.to(self.m_device)
            neg_tag_batch_gpu = neg_tag_batch.to(self.m_device)
            rating_batch_gpu = rating_batch.to(self.m_device)

            user_batch_gpu = user_batch.to(self.m_device)
            item_batch_gpu = item_batch.to(self.m_device)
            #tag_batch_gpu1 = tag_batch1.to(self.m_device)
            #tag_batch_gpu2 = tag_batch2.to(self.m_device)

            #logits, user_x, item_x, pos_user_tag_x, pos_item_tag_x, neg_user_tag_x, neg_item_tag_x = network(pos_tag_batch_gpu, neg_tag_batch_gpu, rating_batch_gpu, user_batch_gpu, item_batch_gpu)
            user_attr_p, user_attr_t, item_attr_p, item_attr_t, rating_2 = network(pos_tag_batch_gpu, neg_tag_batch_gpu, rating_batch_gpu, user_batch_gpu, item_batch_gpu )
            ...
            NLL_loss = self.m_rec_loss(user_attr_p, user_attr_t, item_attr_p, item_attr_t, rating_2)
    ...
    def f_eval_epoch(self, eval_data, network, optimizer, logger_obj):
        ...
        with torch.no_grad():
            for pos_tag_batch, mask_batch, user_batch, item_batch in eval_data:
                ...
                user_logits, item_logits = self.f_get_pred(network, user_batch_gpu, item_batch_gpu)
                precision, recall = get_precision_recall(user_logits.cpu(), item_logits.cpu(), pos_tag_batch, mask_batch, k=3)
