...
class _MOVIE(Dataset):
    def __init__(self, args, df):
        ...
        pos_tag_list = df.user_attr.tolist()
        neg_tag_list = df.item_attr.tolist()
        #pos_tag_list = user_attrs.tolist()
        #neg_tag_list = item_attrs.tolist()
        rating_list = df.rating.tolist()
        #tag1_list = df.tagid1.tolist()
        #tag2_list = df.tagid2.tolist()
        ...
        # for sample_index in range(1000):
            ...
            rating_id = rating_list[sample_index]
            #tag_id1 = tag1_list[sample_index]
            #tag_id2 = tag2_list[sample_index]


            
            self.m_pos_tag_batch_list.append(pos_tag_id)
            self.m_neg_tag_batch_list.append(neg_tag_id)
            self.m_rating_list.append(rating_id)
            #self.tag1_list.append(tag_id1)
            #self.tag2_list.append(tag_id2)
    ...
    def __getitem__(self, idx):
        ...
        rating_i = self.m_rating_list[i]

        #tag1_i = self.tag1_list[i]
        #tag2_i = self.tag2_list[i]

        return pos_tag_i, neg_tag_i, rating_i, user_i, item_i
    ...
    def collate(batch):
        ...
        for i in range(batch_size):
            ...
            #tag1_i = sample_i[5]
            #tag1_iter.append(tag1_i)

            #tag2_i = sample_i[6]
            #tag2_iter.append(tag2_i)
        ...
        rating_tensor = torch.from_numpy(np.array(rating_iter)).long()
        #tag_tensor1 = torch.from_numpy(np.array(tag1_iter)).long()
        #tag_tensor2 = torch.from_numpy(np.array(tag2_iter)).long()

        
        user_iter_tensor = torch.from_numpy(np.array(user_iter)).long()
        item_iter_tensor = torch.from_numpy(np.array(item_iter)).long()

        return pos_tag_iter_tensor, neg_tag_iter_tensor, rating_tensor, user_iter_tensor, item_iter_tensor
...
class _MOVIE_TEST(Dataset):
    def __init__(self, args, df):
        ...
        #pos_tag_lost = df.pos_attr.tolist()
